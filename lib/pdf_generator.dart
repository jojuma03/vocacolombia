...
_PerfilTemperamento _obtenerPerfil(String area) {
  ...
  if (a.contains('realista') ||
      a.contains('tecnologia')) {
    return const _PerfilTemperamento(
      'Racional y Logico',
      'Metodico y Estructurado',
    );
  }

  if (a.contains('investigador') ||
      a.contains('salud')) {
    return const _PerfilTemperamento(
      'Empatico y Servicial',
      'Cuidadoso y Observador',
    );
  }

  if (a.contains('artistico') ||
      a.contains('arte')) {
    return const _PerfilTemperamento(
      'Creativo y Expresivo',
      'Innovador y Sensible',
    );
  }

  if (a.contains('social') ||
      a.contains('sociales')) {
    return const _PerfilTemperamento(
      'Comunicativo y Justo',
      'Lider y Persuasivo',
    );
  }

  if (a.contains('emprendedor') ||
      a.contains('negocios')) {
    return const _PerfilTemperamento(
      'Emprendedor y Competitivo',
      'Estrategico y Organizado',
    );
  }
...
Future<void> generarPDF({
  ...
  final logoData = await rootBundle.load(
    'assets/images/logo_logoterapia.png',
  );
  ...
  if (kIsWeb) {
    await Printing.sharePdf(
      bytes: bytes,
      filename:
      'reporte_vocacional_${nombre.replaceAll(' ', '_')}.pdf',
    );
  } else {
